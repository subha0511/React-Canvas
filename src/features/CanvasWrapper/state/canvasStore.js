...
export const useCanvasStore = create(
  log((set, get) => ({
    ...
    //Varies for different animations like zoom, pan, focus and pan,
    ...
    actions: {
      ...
      updateCanvasRange: () => {
        ...
        const x = [
          -width * scale + window.innerWidth / 2,
          window.innerWidth / 2,
        ];
        const y = [
      ...
      onMouseDrag: (event) => {
        ...
        if (isMouseDown) {
          ...
          get().actions.translate({ dX, dY });
          get().actions.updateCrosshair();
        }
      },
      ...
      focusPan: (event) => {
        ...
        get().actions.translate({ dX, dY });
        get().actions.updateCrosshair();
        get().actions.setCellFocused(true);
      },
      ...
      onKeyboardScale: ({ d_y }) => {
        ...
        const dY =
          ((window.innerHeight / 2 - cy) * (scale - new_scale)) / scale;
